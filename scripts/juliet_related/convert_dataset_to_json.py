...
class DatasetConverter:
    ...
    def extract_vulnerable_code(self, java_file_path: Path) -> Optional[str]:
        ...
        try:
            ...
            print(
                f"Warning: Failed to extract vulnerable code from {java_file_path}: {e}"
            )
            return None
...
Please provide only the replacement code:"""
    ...
    def process_testcase_variant(
        self, testcase_dir: Path, variant: str
        ...
        try:
            ...
            if vulnerable_code is None:
                print(
                    f"Warning: Could not extract vulnerable code from {original_file}"
                )
                vulnerable_code = ""

            # Generate prompt
            input_prompt = self.generate_completion_prompt(
                masked_content, description_content
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Convert juliet-java dataset to JSON format"
    )
    parser.add_argument(
        "--dataset-path",
        default="dataset",
    ...
    parser.add_argument(
        "--output",
        default="juliet_java_dataset.json",
